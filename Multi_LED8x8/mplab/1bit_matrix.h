...
//     printf("(1_bit_matrix_get_bit) index out of bound (max_row = %d, max_col = %d)\n", max_row, max_col);
     if(row >= max_row)
     {
         dbg_print2(1, "ERROR: (1_bit_matrix_set_bit) index out of bound (row = %d, max_row = %d)", row, max_row);
         exit(-1);
     }              
     
     if(col >= max_col)
     {
         dbg_print2(1, "ERROR: (1_bit_matrix_set_bit) index out of bound (col = %d, max_col = %d)\n", col, max_col);
         exit(-1);
     }              
...
//     fprintf(fp, "(1_bit_matrix_set_bit) index out of bound (max_row = %d, max_col = %d)\n", max_row, max_col);
     if(row >= max_row)
     {
         dbg_print2(1, "ERROR: (1_bit_matrix_set_bit) index out of bound (row = %d, max_row = %d)\n", row, max_row);
         exit(-1);
     }              
